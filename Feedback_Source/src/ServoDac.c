...
#define S_FUNCTION_NAME	ServoDac
...
static void mdlStart(SimStruct *S)
{
    if (ACCESS_HW && (ssGetSimMode(S) != SS_SIMMODE_RTWGEN)) {
		...
		// zero the dac output
		servoTrainer_writeToDac (baseAddr, mode, outValue);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
  ...
  if (ACCESS_HW && (ssGetSimMode(S) != SS_SIMMODE_RTWGEN)) 
  {
	  ...
	  // MODE = 1 for RawData mode and 2 for Voltage mode
	  if (MODE ==2) mode = 1;
	  else mode = 0;
 
	  servoTrainer_writeToDac (baseAddr, mode, value);
