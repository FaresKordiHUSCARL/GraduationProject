...
#define S_FUNCTION_NAME	pci1711da
...
static void mdlStart(SimStruct *S)
{
    ...
    if (ACCESS_HW && (ssGetSimMode(S) != SS_SIMMODE_RTWGEN)) {
		...
		pci1711_writeToDac(baseAddr, OUT_CHANNEL, RANGE_OUT, 0.0);
...
static void mdlTerminate(SimStruct *S)
{
	...
	if (ACCESS_HW && (ssGetSimMode(S) != SS_SIMMODE_RTWGEN)) 
	{
		...
		pci1711_writeToDac(baseAddr, 0, RANGE_OUT, 0.0);
		pci1711_writeToDac(baseAddr, 1, RANGE_OUT, 0.0);
